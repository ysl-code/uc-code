...
import os
#from envs.lbforaging.foraging.environment import
import random
class ForagingEnv(MultiAgentEnv):
    ...
    def __init__(self,
        ...
        entry_point="envs.lbforaging.foraging.environment:ForagingEnv",
        kwargs={
            "players": players,
            "max_player_level": 3,
        ...
        env_id = "Foraging{4}-{0}x{0}-{1}p-{2}f{3}-v0".format(field_size, players, max_food,
                                                              "-coop" if force_coop else "",
                                                              "-{}s".format(sight) if partially_observe else "") #10,4,4,false,2

        # 动态生成环境 ID，添加随机数或进程编号
        #unique_id = random.randint(1000, 9999)  # 随机数
        env_id = "Foraging{4}-{0}x{0}-{1}p-{2}f{3}-v{5}".format(
            field_size, players, max_food,
            "-coop" if force_coop else "",
            "-{}s".format(sight) if partially_observe else "",
            id #unique_id  # 将随机数作为版本号
        )
        ...
        print('env_id:',env_id)  #env_id: Foraging-2s-10x10-4p-4f-v0
        print(env_id)
    ...
    def step(self, actions):
        ...
        self.obs, rewards, dones, info, self.food_state, self.player_state = self.env.step(actions)
    ...
    def get_alive_agents(self):
        alive_allies_list = np.ones([self.n_agents])
    ...
    def get_env_info(self):
        ...
        env_info = {
            ...
            #'agent_num_infos': self.get_agent_num_infos(),
        }
        return env_info
