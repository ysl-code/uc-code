...
class DDPGAgent(object):
    ...
    def step(self, obs, explore=False):
        ...
        """
        action = self.policy(obs)   #(4,32)
        if explore:
            action = gumbel_softmax(action, hard=True)
        else:
            action = onehot_from_logits(action)
        #print('action.shape:',action.shape)  #(4,5),(4,5)
        return action
...
def run(_run, _config, _log):
    ...
    if args.env == 'sc2' or args.env =='dec_tiger':
        name1 = args.name+'_map_' + args.env_args['map_name']+'_run_'+str(args.batch_size_run)+'_buffer_'+str(args.buffer_size)+'_seed_'+str(args.seed)+'_serial'
    elif args.env == "foraging":
        name1 = args.name + '_map_' + args.env + '_' + args.temp_id + '_run_' + str(args.batch_size_run) + '_buffer_' + str(args.buffer_size) + '_seed_' + str(args.seed) + '_serial'
    else:
        name1 = args.name + '_map_' + args.env + '_' + args.scenario_name + '_run_' + str(args.batch_size_run) + '_buffer_' + str(args.buffer_size) + '_seed_' + str(args.seed) + '_serial'

    unique_token = "{}__{}".format(name1, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    ...
    if args.env == 'mpe_env':
        ...
        for agent_idx in range(num_agents):
            if is_opponent(args.scenario_name, agent_idx, num_cooperating_agents, num_opponent_agents):
                ...
                #print('num_in_pol:',env_info['observation_space'][agent_idx].shape[0])
                #print('num_out_pol:',env_info['action_space'][agent_idx].n)
        opponents = [DDPGAgent(**params) for params in opponent_init_params]

        pretrained_opponent_model_dir = './pretrained_models/{}_{}_episode_{}.pt'.format(
            args.scenario_name,
            args.opponent_algo,
