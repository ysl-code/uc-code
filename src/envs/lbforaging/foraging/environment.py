...
class ForagingEnv(Env):
    ...
    def spawn_food(self, max_food, max_level):
        ...
        while food_count < max_food and attempts < 1000:
            ...
            self.food_state.append(row)
            self.food_state.append(col)
            self.food_state.append(levels[food_count])
            food_count += 1
        ...
        for player in self.players:
            ...
            while attempts < 1000:
                ...
                if self._is_empty_location(row, col):
                    ...
                    self.player_state.append(row)
                    self.player_state.append(col)
                    self.player_state.append(levels[i])
                    i += 1
                    break
                attempts += 1
    ...
    def _make_gym_obs(self, observations):
        def make_obs_array(observation):
            ...
            #print('self.observation_space[0].shape:',self.observation_space[0])
        ...
        return nobs, nreward, ndone, ninfo, self.food_state, self.player_state
    ...
    def reset(self):
        ...
        nobs, nreward, ndone, ninfo, _, _ = self._make_gym_obs(observations)
        return nobs, self.food_state, self.player_state
    ...
    def step(self, actions):
        ...
        while loading_players:
            ...
            for i in range(self.max_food):
                if self.food_state[3 * i] == frow and self.food_state[3 * i + 1] == fcol:
                    self.food_state[3 * i] = 0
                    self.food_state[3 * i + 1] = 0
                    self.food_state[3 * i + 2] = 0
                    break
        ...
        for i, p in enumerate(self.players):
            p.score += p.reward
            self.player_state[3 * i] = p.position[0]
            self.player_state[3 * i + 1] = p.position[1]
