...
class GumbleSoftmax(torch.nn.Module):
    ...
    def sample_gumbel_like(self, template_tensor, eps=1e-10):
        ...
        gumble_samples_tensor = - torch.log(eps - torch.log(uniform_samples_tensor + eps))  #引入eps:为了防止出现0的情况从而出现计算错误
    ...
    def gumbel_softmax_sample(self, logits, test):
        ...
        soft_samples = F.softmax(gumble_trick_log_prob_samples / self.temp, dim)  #self.temp=1
        return soft_samples
    ...
    def gumbel_softmax(self, logits, hard=False, test=False):
        ...
        """
        y = self.gumbel_softmax_sample(logits, test)  #(n_nodes*n_nodes,2)
        if hard:
