...
class TransformerModule(nn.Module):
    ...
    def forward(self, src: Tensor) -> Tensor:
        seq_length = src.size(1)  #(8576,10,64)
        batch_size = src.size(0)
        src = self.layer_norm(src)
        all_Ks = [K(src) for K in self.Ks] #[(8576,10,64),(8576,10,64),(8576,10,64),(8576,10,64)]
        ...
        for K, Q, V in zip(all_Ks, all_Qs, all_Vs):
            ...
            softmax_QK = F.softmax(norm_QK, dim=-1)  #(8576,10,10)
            attention_value = torch.bmm(softmax_QK, V) #(8576,10,64)
            attention_values.append(attention_value) #[(8576,10,64),(8576,10,64),(8576,10,64),(8576,10,64)] # [batch_size, seq_len, ntoken]
        transformer_output = torch.stack(attention_values).sum(0).view(batch_size, seq_length, -1)  #(8576,10,64)
        #transformer_output = self.decoder(transformer_output).mean(1)
        #return self.output(transformer_output)
        return self.tanh(transformer_output) #(8576,10,64)
